%% RGB split of picture

clear
close all;

im1 = imread('Beautiful_Green_Picture.jpg');
figure
imshow(im1)
imwrite(im1,'origninalImage.png')
...
%% Lab 1 part 1 seems to have changed from when I read it

clear
close all;

% read in simple images
oneColour = imread("One_colour.jpg");
twoColour = imread("Two_colour.jpg");

% Convert to greyscale
oneColour = rgb2gray(oneColour);
twoColour = rgb2gray(twoColour);

figure
subplot(2,2,1), imshow(oneColour), title('One colour image')
subplot(2,2,3), imshow(twoColour), title('Two colour image')
subplot(2,2,2), imhist(oneColour), title('Histogram of greyscale one colour image')
subplot(2,2,4), imhist(twoColour), title('Histogram of greyscale two colour image')
...
imwrite(fruits,'imageFruits.png')
...
SandPminusMedian = medfilt2(fruitSandPnoise, [3 3]); % [3 3] is optimal
figure
